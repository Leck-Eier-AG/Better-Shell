...
<task type="auto">
  ...
  <action>
    ...
    **vendor/bash-preexec.sh:**
    - Download bash-preexec v0.6.0 from the pinned URL: https://raw.githubusercontent.com/rcaloras/bash-preexec/0.6.0/bash-preexec.sh
    - Use `curl -fsSL` to fetch it. If download fails, create a placeholder with a comment explaining manual download needed and the exact URL above.
    - Pin the version with a comment at the top of the file: `# bash-preexec v0.6.0 — https://raw.githubusercontent.com/rcaloras/bash-preexec/0.6.0/bash-preexec.sh`
  </action>
  <verify>
  ...
  <done>
    Makefile exists with build/clean targets. lib/header.sh has idempotency guard and variable init. lib/env.sh has _bsh_is_interactive() with interactive, TTY, SSH, and CI checks. lib/compat.sh detects bash vs zsh and sets up bash-preexec sourcing. vendor/bash-preexec.sh is downloaded (v0.6.0) and pinned.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    **Test isolation — concrete sourcing approach for non-interactive scripts:**

    Since the test script is non-interactive, sourcing env.sh normally would hit the `_bsh_is_interactive || return 0` gate and skip all subsequent code. Use the following two-phase approach:

    Phase A — Test `_bsh_is_interactive` in isolation:
    ```bash
    # Source ONLY header.sh (for _BSH_SSH_ENABLED default)
    source lib/header.sh
    # Source env.sh — the function definition loads, then the gate triggers return 0.
    # But the function IS defined before the gate, so we can test it afterward:
    source lib/env.sh
    # _bsh_is_interactive is now defined (the return 0 only exits env.sh scope, not the test script)
    # Test it: should return 1 since we are non-interactive
    _bsh_is_interactive; [[ $? -eq 1 ]] && _test_pass "..." || _test_fail "..."
    ```

    Phase B — Test hooks/compat by manually setting required variables and sourcing past the gate:
    ```bash
    # Reset for hook testing — pre-set the variables that env.sh's gate would skip setting
    _BSH_ENABLED=1
    _BSH_SSH_ENABLED=0
    _BSH_SHELL="bash"
    # Source compat.sh (needs _BSH_SHELL already set; it will try to set it too, which is fine)
    source lib/compat.sh
    # Source hooks.sh (needs _BSH_SHELL for registration dispatch)
    source lib/hooks.sh
    # Now _bsh_preexec and _bsh_precmd are defined and registered — test them directly
    ```

    This ensures tests do NOT trivially pass by accidentally skipping the code under test. Each test case calls the function and checks a concrete postcondition.
